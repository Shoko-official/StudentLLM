...
VAD uses a 1,000 ms minimum silence and 400 ms speech padding to retain word edges. The decoder keeps the usual no-speech threshold `0.6`, log-probability threshold `-1.0`, and compression-ratio threshold `2.4`. Word alignment is enabled in both modes to support the 2-second hallucination-silence filter; only previews serialize the word payload. These controls reduce some failure modes but do not guarantee silence detection or remove every hallucination. Disabling previous-text conditioning can also reduce consistency across windows. Parameter semantics are documented in the [faster-whisper transcription implementation](https://github.com/SYSTRAN/faster-whisper/blob/v1.2.1/faster_whisper/transcribe.py).
