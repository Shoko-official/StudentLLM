...
| --- | --- | --- |
| Application verification | `npm run verify` | TypeScript, benchmark adapter checks, 87 Vitest tests, Vite production build, and 31 Playwright tests pass |
...
| Tauri desktop shell | Rust check/tests, cross-platform debug builds, unsigned package jobs, packaged launch and crash-recovery smoke, manual recovery soak, and managed sidecar lifecycle | Shell scaffold, SQLite WAL workspace bridge, managed local sidecar lifecycle commands, real child-process start/stop and exited-process recovery coverage pass native compilation/tests in CI; unsigned Ubuntu, Windows, and macOS packages now all launch, persist a frontend-generated workspace through native IPC, verify a frontend-to-native command round trip, survive a forced stop and ten relaunch cycles, pass SQLite integrity validation, and verify two configured sidecars start and stop in the packaged runtime. The dedicated 30-cycle soak passed on Ubuntu, Windows, and macOS in workflow [33290586153](https://github.com/Shoko-official/StudentLLM/actions/runs/33290586153) |
