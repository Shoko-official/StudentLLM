...
| BIG-Bench Hard | Four official tasks, 250 public cases per task through NVIDIA NIM | `logical_deduction_seven_objects`: flexible-extract `59.20%`; `multistep_arithmetic_two`: flexible-extract `96.40%`, strict-match `64.80%`; `tracking_shuffled_objects_seven_objects`: flexible-extract `85.20%`; `dyck_languages`: flexible-extract `3.60%`, strict-match `3.20%`; four task samples, not a full BBH result |
