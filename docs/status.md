...
| BIG-Bench Hard | Two official tasks, 250 public cases per task through NVIDIA NIM | `logical_deduction_seven_objects`: flexible-extract `59.20%`; `multistep_arithmetic_two`: flexible-extract `96.40%`, strict-match `64.80%`; two task samples, not a full BBH result |
