...
| NVIDIA integration | Live provider smoke and official BFCL samples | NIM request passes using the Windows User `NVIDIA_API_KEY` environment variable; seven BFCL public category samples completed |
...
| BIG-Bench Hard | Three official tasks, 250 public cases per task through NVIDIA NIM | `logical_deduction_seven_objects`: flexible-extract `59.20%`; `multistep_arithmetic_two`: flexible-extract `96.40%`, strict-match `64.80%`; `tracking_shuffled_objects_seven_objects`: flexible-extract `85.20%`; three task samples, not a full BBH result |
