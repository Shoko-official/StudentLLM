...
| LM Studio integration | Existing `llama-server` process, live provider smoke, browser chat, and public DROP probe | The existing process is reachable on `127.0.0.1:19248`; live chat and a one-example DROP probe pass. A larger local DROP run was interrupted before writing a receipt and is not counted as a score |
