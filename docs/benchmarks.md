...
| NVIDIA generation | Live API, runtime credential from the Windows User environment | `npm run providers:smoke` | PASS observed on 2026-08-29, 1,207 ms |
| LM Studio generation | Live local server, existing process | `npm run providers:smoke` | PASS observed on 2026-08-29, 61,841 ms with `openai/gpt-oss-20b`; the existing model process was not restarted |
...
| `live_irrelevance`, 128 of 884 public cases | Irrelevance detection `0.9688` (124/128) | Mean `6.534 s` | Public partial sample; 128 unique result rows, with 124 empty result arrays consistent with no-tool decisions |
