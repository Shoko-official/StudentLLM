...
| AMI meeting speech ASR | Complete public `sdm/test` split plus first 200 public `ihm/test` segments, faster-whisper large-v3-turbo on local CUDA | `benchmarks/run_asr_hf.py --dataset edinburghcstr/ami --config sdm --split test --reference-field text --language en --model large-v3-turbo --streaming --device cuda --compute-type float16` | Complete SDM WER 0.4083 / CER 0.3142; IHM WER 0.2362 / CER 0.1634; RTF 0.0684 / 0.0887; far-field target unmet |
...
| 2026-09-01 | `ihm` / faster-whisper, CUDA `float16` | 200 public test segments, 1,067 reference words, 383.87 seconds of decoded audio | WER `23.6176%`, CER `16.3377%`, RTF `0.0887`, elapsed `34.04s` | Windows 11, RTX 5080, 16,303 MiB VRAM; partial first-200 public segments |

The complete SDM measurement remains above the current ASR quality target and confirms a substantial acoustic penalty for distant meeting audio. Diarization and speaker-attributed WER metrics remain outstanding.
