...
| AMI meeting speech ASR | First 200 public `sdm` and `ihm` test segments, faster-whisper large-v3-turbo on local CUDA | `benchmarks/run_asr_hf.py --dataset edinburghcstr/ami --config sdm --split test --reference-field text --language en --model large-v3-turbo --limit 200 --streaming --device cuda --compute-type float16` | SDM WER 0.5714 / CER 0.4610; IHM WER 0.2362 / CER 0.1634; RTF 0.0967 / 0.0887; partial public comparison |
...
The generic Hugging Face adapter evaluated the first 200 public segments from the `edinburghcstr/ami` `test` split with `faster-whisper large-v3-turbo`, CUDA `float16`, beam size 5, and VAD filtering. The `sdm` configuration uses a single distant microphone; the `ihm` configuration uses individual headset microphones. The adapter's streaming mode keeps large public splits from being materialized in memory. These are partial ASR comparisons, not full AMI scores and not diarization or speaker-attributed WER measurements.

| Run | Microphone condition | Evaluation set | Result | Hardware and validity |
| --- | --- | --- | --- | --- |
| 2026-09-01 | `sdm` / faster-whisper, CUDA `float16` | 200 public test segments, 1,050 reference words, 362.30 seconds of decoded audio | WER `57.1429%`, CER `46.0989%`, RTF `0.0967`, elapsed `35.02s` | Windows 11, RTX 5080, 16,303 MiB VRAM; partial first-200 public segments |
...
The microphone comparison shows a substantial acoustic penalty for distant meeting audio. Both partial measurements remain above the current ASR quality target, and the complete AMI split plus diarization metrics remain outstanding.
