...
| Earnings-22 long-form ASR | Complete public `distil-whisper/earnings22` `chunked/test` split, faster-whisper large-v3-turbo on local CUDA | `benchmarks/run_asr_hf.py --dataset distil-whisper/earnings22 --config chunked --split test --reference-field transcription --language en --model large-v3-turbo --streaming --device cuda --compute-type float16` | WER 0.2060 / CER 0.1428 / RTF 0.0320; 57,391 examples; 415,566.96 seconds of audio; complete public split, target unmet |
| CoVoST 2 speech translation | Complete public `fixie-ai/covost2` `fr_en/test` split, official Facebook speech translation model on local CUDA | `benchmarks/run_covost2_st.py --dataset fixie-ai/covost2 --config fr_en --split test --device cuda --compute-type float32`; add `--comet-model Unbabel/wmt22-comet-da` for COMET | Full split: BLEU 0.2631 / chrF 0.5353 / COMET 0.7068 / RTF 0.0543 across 14,760 examples and 83,894.74 seconds of audio. Separate 1,000-example COMET diagnostic: 0.6689 |
...
| DocVQA ANLS vision evaluation, larger subset | Public DocVQA validation images through NVIDIA `meta/llama-3.2-11b-vision-instruct` | `benchmarks/run_docvqa_anls.py --split validation --limit 500 --concurrency 4` | Official ANLS `0.8297` on 498/500 successful public validation examples; larger partial subset and selected baseline |
| DocVQA ANLS vision evaluation, 1,000-image subset | Public DocVQA validation images through NVIDIA `meta/llama-3.2-11b-vision-instruct` | `benchmarks/run_docvqa_anls.py --split validation --limit 1000 --concurrency 4` | Official ANLS `0.8321` on 988/1,000 successful public validation examples; largest measured partial subset and selected baseline |
...
| Rich text and formula rendering | Bundled KaTeX HTML plus MathML, accessible labels, and provider-state UI | Vitest plus browser Playwright coverage | PASS; inline and block formulas render in transcript, course notes, chat, and artifact surfaces with semantic MathML; the browser smoke observed zero page or console errors |
| NVIDIA generation | Live API, runtime credential from the Windows User environment | `npm run providers:smoke` | PASS observed on 2026-09-16 with `openai/gpt-oss-20b`, 8,910 ms |
| LM Studio generation | Live local server, existing process | `npm run providers:smoke` | PASS observed on 2026-09-16 with `openai/gpt-oss-20b`, 1,263 ms; the server and loaded model were started for this validation; one bounded retry now covers transient engine-protocol failures |
...
| BEIR SciFact retrieval | Full public test split, deterministic BM25 | `benchmarks/run_beir_bm25.py --dataset scifact --output_path artifacts/benchmarks/beir/scifact-bm25-20260916.json` | nDCG@10 0.6593, Recall@10 0.7809, MRR@10 0.6252; 1,109 query rows / 300 evaluated queries |
...
| BEIR SciFact encoder comparison | Full public test split, BGE-base, BGE-large, BGE-M3, and E5-large normalized embeddings on local CUDA | `benchmarks/run_beir_dense.py --dataset scifact --model <model> --device cuda --batch-size 32 --query-prefix <prefix> --document-prefix <prefix>` | Fresh 2026-09-16 instructed BGE-large receipt: nDCG@10 0.7463 / Recall@10 0.8732 / MRR@10 0.7124 in 57.233 seconds; it matches the strongest prior measured profile. Instructed BGE-base remains 0.7404 / 0.8742 / 0.7034; E5-large with `query:` and `passage:` prefixes scored 0.7222 / 0.8438 / 0.6878; BGE-M3 at 512 tokens scored 0.6415 / 0.7751 / 0.6080 |
| BEIR SciFact hybrid retrieval | Full public test split, fixed BM25 plus BGE-large reciprocal-rank fusion | `benchmarks/run_beir_hybrid.py --dataset scifact --model BAAI/bge-large-en-v1.5 --device cuda --batch-size 64 --candidate-k 100` | Fresh 2026-09-16 receipt: nDCG@10 0.7220 / Recall@10 0.8329 / MRR@10 0.6931; below dense-only BGE-large, so hybrid is not promoted |
| BEIR SciFact reranking comparison | Full public test split, BGE-large top-100 retrieval plus public CrossEncoder reranking | `benchmarks/run_beir_rerank.py --dataset scifact --model BAAI/bge-large-en-v1.5 --reranker-model BAAI/bge-reranker-large --device cuda --batch-size 64 --rerank-batch-size 32 --candidate-k 100` | Fresh 2026-09-16 receipt: nDCG@10 0.7379 / Recall@10 0.8548 / MRR@10 0.7067 in 459.806 seconds; below matched instructed dense-only BGE-large, so it is not promoted |
...
## Observed public result: AMI meeting speech ASR

The generic Hugging Face adapter evaluated the complete public `sdm/test` and `ihm/test` splits with `faster-whisper large-v3-turbo`, CUDA `float16`, beam size 5, and VAD filtering. The `sdm` configuration uses a single distant microphone; the `ihm` configuration uses individual headset microphones. The adapter's streaming mode keeps large public splits from being materialized in memory. These are ASR measurements, not diarization or speaker-attributed WER measurements.

| Run | Microphone condition | Evaluation set | Result | Hardware and validity |
| --- | --- | --- | --- | --- |
| 2026-09-01 | `sdm` / faster-whisper, CUDA `float16` | 12,643 public test segments, 90,145 reference words, 31,253.32 seconds of decoded audio | WER `40.8331%`, CER `31.4221%`, RTF `0.0684`, elapsed `2,136.18s` | Windows 11, RTX 5080, 16,303 MiB VRAM; complete public `sdm/test` split |
| 2026-09-01 | `ihm` / faster-whisper, CUDA `float16` | 12,643 public test segments, 90,145 reference words, 31,253.32 seconds of decoded audio | WER `20.9307%`, CER `13.4698%`, RTF `0.0682`, elapsed `2,132.12s` | Windows 11, RTX 5080, 16,303 MiB VRAM; complete public `ihm/test` split |
...
| Run | Model and backend | Evaluation set | Result | Hardware and validity |
| --- | --- | --- | --- | --- |
| 2026-09-01 | `large-v3-turbo` / faster-whisper, CUDA `float16` | First 1,000 public `chunked/test` segments, 18,262 reference words, 6,767.08 seconds of audio | WER `17.2654%`, CER `11.4494%`, RTF `0.0659`, elapsed `446.09s` | Windows 11, RTX 5080, 16,303 MiB VRAM; robust partial public sample |
| 2026-09-02 | `large-v3-turbo` / faster-whisper, CUDA `float16` | Complete public `chunked/test` split, 57,391 examples, 1,018,570 reference words, 415,566.96 seconds of audio | WER `20.5965%`, CER `14.2776%`, RTF `0.0320`, elapsed `13,303.49s` | Windows 11, RTX 5080, 16,303 MiB VRAM; complete public split, receipt SHA-256 `73D771877B4F6397E1FA83D56B3AEC30A09247E30CDC3AB5D9A866CCE135EA2B` |

The complete split is materially harder than read-speech benchmarks and remains above the current ASR quality target. The full receipt is `artifacts/benchmarks/asr/earnings22-large-v3-turbo-cuda-full.json` and is ignored by Git. The earlier 1,000-segment receipt remains available as a comparable partial sample.

## Observed public result: CoVoST 2 speech translation

The 2026-09-01 complete public `fixie-ai/covost2` `fr_en/test` run used the official `facebook/s2t-small-covost2-fr-en-st` checkpoint, PyAV decoding, CUDA FP32 inference on an NVIDIA GeForce RTX 5080, and SacreBLEU plus chrF scoring. All 14,760 public examples were evaluated: BLEU `0.263113`, chrF `0.535307`, 83,894.74 seconds of audio, 5,313.33 seconds elapsed, and RTF `0.063333`. The full receipt is `artifacts/benchmarks/covost2/facebook-s2t-small-fr-en-cuda-float32-full.json`, ignored by Git; its SHA-256 is `D4A17B74133AD626EF33D800188460D7B2779A790344FD94B67FCF3DF9107AD2`.

A separate 1,000-example run added the optional `Unbabel/wmt22-comet-da` scorer and returned COMET `0.668898`, BLEU `0.228565`, chrF `0.488345`, and RTF `0.066166`, with 1,000/1,000 provider evaluations completed. This remains a useful partial diagnostic. The complete split was then evaluated with the same scorer: COMET `0.706765`, BLEU `0.263113`, chrF `0.535308`, 83,894.74 seconds of audio, 4,557.37 seconds elapsed, and RTF `0.054322`. The full COMET receipt is `artifacts/benchmarks/covost2/facebook-s2t-small-fr-en-cuda-float32-comet-full.json`, ignored by Git; its SHA-256 is `69A2FB77CE05E3B46ABDDA9C0B9907A01BABC68DFE85602CF8944D0FDFB656AD`.
...
The 2026-09-04 reranking comparison used the complete SciFact test split, BGE-base dense top-100 candidates, and two public CrossEncoders. `cross-encoder/ms-marco-MiniLM-L-6-v2` scored nDCG@10 `0.694969`, Recall@10 `0.822222`, and MRR@10 `0.663997` in `54.439` seconds. `BAAI/bge-reranker-base` scored nDCG@10 `0.713320`, Recall@10 `0.840111`, and MRR@10 `0.679505` in `206.055` seconds. Both were below the matched dense-only BGE-base result, so neither is promoted. The runner remains available for future model comparisons without changing the application retrieval path.

The fresh 2026-09-16 SciFact comparison kept the official public test split and qrels. Deterministic BM25 scored nDCG@10 `0.659293`, Recall@10 `0.780944`, and MRR@10 `0.625181` in `8.807` seconds. Uninstructed BGE-large dense retrieval scored `0.734632` / `0.859222` / `0.701276` in `65.286` seconds; fixed BM25 + dense reciprocal-rank fusion scored `0.722030` / `0.832889` / `0.693071` in `58.912` seconds. The model-card query instruction `Represent this sentence for searching relevant passages: ` restored the strongest profile at nDCG@10 `0.746305`, Recall@10 `0.873222`, and MRR@10 `0.712402` in `57.233` seconds. BGE-large plus `BAAI/bge-reranker-large` scored `0.737909` / `0.854778` / `0.706653` in `459.806` seconds, below the instructed dense profile. The fresh receipts are `artifacts/benchmarks/beir/scifact-bm25-20260916.json`, `artifacts/benchmarks/beir/scifact-bge-large-cuda-20260916.json`, `artifacts/benchmarks/beir/scifact-bge-large-hybrid-20260916.json`, `artifacts/benchmarks/beir/scifact-bge-rerank-large-20260916.json`, and `artifacts/benchmarks/beir/scifact-bge-large-instructed-cuda-20260916.json`.
...
The complete reference-only campaign was then run on 2026-09-14 with one worker, the full available task context, `max-tokens 512`, and the same NVIDIA `openai/gpt-oss-20b` endpoint. The official `reference.jsonl` file was evaluated by IBM's algorithmic evaluator after generation:

| Metric | Aggregate | Tasks | Non-empty predictions | Generation errors |
| --- | ---: | ---: | ---: | ---: |
| Recall | `0.401087` | 842 | 837 | 5 |
| ROUGE-L (unstemmed) | `0.268139` | 842 | 837 | 5 |
| BERTScore precision | `0.193058` | 842 | 837 | 5 |
...
The official file contains 1,371 validation examples and 1,335 public examples. The first bounded validation smoke on 2026-09-04 evaluated 10 examples with NVIDIA `openai/gpt-oss-20b`, bounded provider timeouts, and the optional judge. It produced zero generation failures, one correct answer, six missing answers, three incorrect answers, and a conservative judge score of `-0.2000`.
...
The largest measured subset with the same model and protocol scored ANLS `0.8321` on 1,000 public validation examples, with 988 successful responses and 12 retained provider failures, in `504.538` seconds. The provider failures were HTTP 429 responses after the configured retries; they remain visible in the receipt and contribute zero to the reported aggregate. This is the selected partial baseline because it provides the largest measured sample while remaining an explicitly incomplete validation result. The local receipt is `artifacts/benchmarks/docvqa/nvidia-llama-11b-vision-validation-1000.json`; it is ignored by Git.
