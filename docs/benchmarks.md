...
| CoVoST 2 speech translation | Complete public `fixie-ai/covost2` `fr_en/test` split, official Facebook speech translation model on local CUDA | `benchmarks/run_covost2_st.py --dataset fixie-ai/covost2 --config fr_en --split test --device cuda --compute-type float32`; add `--comet-model Unbabel/wmt22-comet-da` for COMET | Full split: BLEU 0.2631 / chrF 0.5353 / RTF 0.0633 across 14,760 examples and 83,894.74 seconds of audio. Separate 1,000-example COMET diagnostic: 0.6689 |
...
A separate 1,000-example run added the optional `Unbabel/wmt22-comet-da` scorer and returned COMET `0.668898`, BLEU `0.228565`, chrF `0.488345`, and RTF `0.066166`, with 1,000/1,000 provider evaluations completed. This is a larger partial COMET diagnostic, not a full-split COMET result. The complete BLEU/chrF score is therefore closed, while full-split COMET remains an open quality gate.
