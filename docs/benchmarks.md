...
| NVIDIA generation | Live API, runtime credential from the Windows User environment | `npm run providers:smoke` | PASS observed on 2026-09-13 with `openai/gpt-oss-20b`, 1,084 ms |
| LM Studio generation | Live local server, existing process | `npm run providers:smoke` | PASS observed on 2026-09-13 with `openai/gpt-oss-20b`, 2,917 ms; the existing model process was not restarted; one bounded retry now covers transient engine-protocol failures |
...
| BEIR SciFact encoder comparison | Full public test split, BGE-base, BGE-large, BGE-M3, and E5-large normalized embeddings on local CUDA | `benchmarks/run_beir_dense.py --dataset scifact --model <model> --device cuda --batch-size 32 --query-prefix <prefix> --document-prefix <prefix>` | Instructed BGE-large is the strongest measured profile: nDCG@10 0.7463 / Recall@10 0.8732 / MRR@10 0.7124; instructed BGE-base scored 0.7404 / 0.8742 / 0.7034; E5-large with `query:` and `passage:` prefixes scored 0.7222 / 0.8438 / 0.6878; BGE-M3 at 512 tokens scored 0.6415 / 0.7751 / 0.6080 |
