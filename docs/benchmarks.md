...
| CoVoST 2 speech translation | First 1,000 public `fixie-ai/covost2` `fr_en/test` examples, official Facebook speech translation model on local CUDA | `benchmarks/run_covost2_st.py --dataset fixie-ai/covost2 --config fr_en --split test --limit 1000 --device cuda --compute-type float32`; add `--comet-model Unbabel/wmt22-comet-da` for COMET | BLEU 0.2286 / chrF 0.4883 / RTF 0.0523; 5,861.93 seconds of audio; partial public sample, full split and COMET receipt open |
