...
The CI matrix builds unsigned packages on Ubuntu, Windows, and macOS. The Ubuntu package job launches the packaged debug executable under Xvfb with isolated data directories, force-stops it after 30 seconds, relaunches it for 15 seconds, and verifies that the same SQLite workspace remains present with a persisted bootstrap snapshot and `PRAGMA integrity_check` returning `ok`. The workspace database is initialized before the Tauri event loop, so persistence setup does not depend on WebView readiness. This verifies packaged Linux startup and native database recovery; it does not replace interactive frontend-to-native IPC validation on each desktop platform or a longer cross-platform recovery soak.
