...
describe('workspace storage', () => {
  ...
  it('round-trips a workspace snapshot through local storage', () => {
    const snapshot: WorkspaceSnapshot = {
      ...
      lessons: [{ ...fallback.lessons[0], id: 'new-lesson', title: 'Cours persistant' }],
      artifacts: [{ id: 'artifact-1', kind: 'summary', label: 'Résumé', createdAt: 'à l’instant' }],
    };
  ...
  it('drops invalid child records instead of restoring corrupt state', () => {
    localStorage.setItem('studentllm.workspace.v1', JSON.stringify({
      ...
      transcript: [{ id: 'ok', timestamp: '00:01', speaker: 'Professeur', text: 'Texte' }, { id: 7 }],
      artifacts: [{ id: 'ok', kind: 'summary', label: 'Résumé', createdAt: 'maintenant' }, { id: 'bad', kind: 'unknown' }],
    }));
